...
std::string find_font_file_by_face_name(const std::wstring &face_name) {
    HKEY fonts_key;
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts", 0, KEY_READ, &fonts_key) != ERROR_SUCCESS) {
        return "";
    }
    ...
    while (true) {
        ...
        LONG status = RegEnumValueW(fonts_key, index, value_name, &value_name_size, nullptr, &type, value_data, &value_data_size);
