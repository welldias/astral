...
int enter_block_callback(MD_BLOCKTYPE type, void* detail, void* userdata) {
  ...
  if (type == MD_BLOCK_LI) {
    if (!ctx->list_stack.empty()) {
      ...
      ctx->blocks->push_back(ContentBlock{
          .kind = BlockKind::ListItem, .list_level = level, .list_index = index, .list_ordered = list.ordered});
    }
    ++ctx->list_item_depth;
    return 0;
  }
...
int leave_block_callback(MD_BLOCKTYPE type, void* /*detail*/, void* userdata) {
  ...
  } else if (type == MD_BLOCK_P && ctx->paragraph_is_bare_image && ctx->paragraph_image_count == 1 &&
             !ctx->blocks->empty()) {
...
void append_text(ParserContext* ctx, const std::string& text) {
  if (ctx->in_table_cell) {
    ...
    ctx->current_table_cell_spans.push_back(TextSpan{text, ctx->bold_depth > 0, ctx->italic_depth > 0,
                                                       ctx->strike_depth > 0, ctx->code_depth > 0});
    return;
  }
  if (ctx->blocks->empty()) {
    return;
  }
  ctx->blocks->back().spans.push_back(TextSpan{text, ctx->bold_depth > 0, ctx->italic_depth > 0,
                                                ctx->strike_depth > 0, ctx->code_depth > 0});
}
