...
int main(int argc, char **argv) {
    ...
    TextRenderer renderer      = load_text_renderer(font_paths, config.font_atlas_base_size, args->emoji_font_path, args->asian_font_path);
    ...
    auto compute_layout_for_slide = [&](int slide_index) {
        ...
        float initial_font_size = config.default_font_size * (static_cast<float>(window_height) / static_cast<float>(config.default_window_height));
    ...
    auto render_slide_into_texture = [&](RenderTexture2D target, const TextLayoutResult &slide_layout, const SlideParams &slide_params, int window_width, int window_height) {
    ...
    while (!WindowShouldClose() && !should_quit) {
        ...
        if (!transition_state.active && !overview_state.active) {
            int next_slide = -1;
            int direction  = 0;
            if ((IsKeyPressed(KEY_RIGHT) || IsKeyPressed(KEY_DOWN)) && current_slide + 1 < static_cast<int>(deck.slides.size())) {
            ...
            if (next_slide != -1) {
                ...
                if (transition != TransitionKind::None) {
                    render_slide_into_texture(transition_from_buffer, layout, deck.slides[current_slide].params, window_width, window_height);
                }
        ...
        if (!overview_state.active && !transition_state.active && is_content_zoomed(zoom_state)) {
            render_slide_into_texture(zoom_buffer, layout, deck.slides[current_slide].params, GetScreenWidth(), GetScreenHeight());
        }
