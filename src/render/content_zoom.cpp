...
void update_content_zoom(ContentZoomState &state, int window_width, int window_height) {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        bool is_double = (now - state.last_click_time) <= kDoubleClickSeconds && (dx * dx + dy * dy) <= (kDoubleClickDistance * kDoubleClickDistance);
        if (is_double) {
            reset_content_zoom(state);
...
void draw_zoomed_content(const ContentZoomState &state, RenderTexture2D source, int window_width, int window_height) {
    ...
    Rectangle dest{ static_cast<float>(window_width) / 2.0f + state.pan_offset.x, static_cast<float>(window_height) / 2.0f + state.pan_offset.y, width, height };
