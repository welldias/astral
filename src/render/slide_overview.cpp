...
Color invert_color(Color color) {
  return Color{static_cast<unsigned char>(255 - color.r), static_cast<unsigned char>(255 - color.g),
               static_cast<unsigned char>(255 - color.b), color.a};
}

int compute_columns(int window_width) {
  int columns = static_cast<int>((static_cast<float>(window_width) + kCellPadding) /
                                  (static_cast<float>(kThumbnailWidth) + kCellPadding));
...
void rebuild_overview_thumbnails(SlideOverviewState& state, const SlideDeck& deck, const TextRenderer& renderer,
  ...
  for (const Slide& slide : deck.slides) {
    TextLayoutResult layout = compute_fitted_layout(slide.content, renderer.fonts, config, initial_font_size,
...
void unload_slide_overview(SlideOverviewState& state) { unload_thumbnail_textures(state); }
