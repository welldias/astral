...
std::vector<int> build_codepoint_set() {
    return base_codepoints();
}
...
RasterizedFont rasterize_sdf_font(std::string font_path, int base_size, std::vector<int> codepoints) {
    ...
    result.glyphs        = LoadFontData(file_data, file_size, base_size, codepoints.data(), static_cast<int>(codepoints.size()), FONT_SDF, &glyph_count);
...
void ensure_styles_loaded(TextRenderer &renderer, const StyleUsage &usage) {
    ...
    auto queue_variant = [&](bool needed, bool &loaded_flag, const std::string &path, Font &out_font, bool &out_distinct) {
    ...
    queue_variant(usage.italic, renderer.italic_loaded, renderer.font_paths.italic, renderer.fonts.italic, renderer.has_distinct_italic_font);
    queue_variant(usage.bold_italic, renderer.bold_italic_loaded, renderer.font_paths.bold_italic, renderer.fonts.bold_italic, renderer.has_distinct_bold_italic_font);
...
void draw_centered_text(const TextRenderer &renderer, const TextLayoutResult &layout, int window_width, int window_height, const AppConfig &config, const SlideParams &params) {
    ...
    for (const LayoutBlock &block : layout.blocks) {
        ...
        float quote_indent    = block.is_block_quote ? block.font_size * (kBlockQuoteBarWidthFraction + kBlockQuoteGapFraction) : 0.0f;
        ...
        if (block.is_list_item) {
            ...
            float marker_width   = MeasureTextEx(renderer.fonts.regular, block.list_marker.c_str(), block.list_marker_font_size, marker_spacing).x;
        ...
        for (size_t line_index = 0; line_index < block.lines.size(); ++line_index) {
            ...
            if (block.is_list_item && line_index == 0 && !block.list_marker.empty()) {
                ...
                positioned.push_back(PositionedRun{ &list_marker_runs.back(), marker_x, marker_y, block.list_marker_font_size, false });
            }
        ...
        if (block.is_code_block) {
            backgrounds.push_back(BackgroundRect{
                Rectangle{ code_box_left_x - code_padding, block_start_y - code_padding, code_box_width + 2.0f * code_padding, (y - block_start_y) + 2.0f * code_padding },
                kCodeBlockRoundness
            });
        }
        if (block.is_block_quote) {
        ...
        if (block.is_table) {
            ...
            backgrounds.push_back(BackgroundRect{
                Rectangle{ table_left_x, table_top_y, table_width, block.table_row_height },
                0.0f
            });
            ...
            table_grid_lines.push_back(Rectangle{ table_left_x + table_width - border * 0.5f, table_top_y, border, table_height });
        }
