...
bool update_and_draw_transition(SlideTransitionState &state, const AppConfig &config, double now, RenderTexture2D from, RenderTexture2D to) {
    ...
    case TransitionKind::Fade: {
        DrawTexturePro(from.texture, from_source, Rectangle{ 0.0f, 0.0f, window_width, window_height }, Vector2{ 0.0f, 0.0f }, 0.0f, WHITE);
        auto alpha = static_cast<unsigned char>(eased * 255.0f);
        DrawTexturePro(to.texture, to_source, Rectangle{ 0.0f, 0.0f, window_width, window_height }, Vector2{ 0.0f, 0.0f }, 0.0f, Color{ 255, 255, 255, alpha });
        break;
    }
    case TransitionKind::Slide: {
        float offset = eased * window_width * static_cast<float>(state.direction);
        DrawTexturePro(from.texture, from_source, Rectangle{ -offset, 0.0f, window_width, window_height }, Vector2{ 0.0f, 0.0f }, 0.0f, WHITE);
        float to_x = static_cast<float>(state.direction) * window_width - offset;
        DrawTexturePro(to.texture, to_source, Rectangle{ to_x, 0.0f, window_width, window_height }, Vector2{ 0.0f, 0.0f }, 0.0f, WHITE);
        break;
    }
    case TransitionKind::Zoom: {
        auto from_alpha = static_cast<unsigned char>((1.0f - eased) * 255.0f);
        DrawTexturePro(from.texture, from_source, Rectangle{ 0.0f, 0.0f, window_width, window_height }, Vector2{ 0.0f, 0.0f }, 0.0f, Color{ 255, 255, 255, from_alpha });
        ...
        DrawTexturePro(to.texture, to_source, Rectangle{ to_x, to_y, scaled_width, scaled_height }, Vector2{ 0.0f, 0.0f }, 0.0f, Color{ 255, 255, 255, to_alpha });
        break;
    }
