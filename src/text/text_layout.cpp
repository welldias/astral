  ...
  for (const TextSpan& span : spans) {
    ...
    while (i < span.text.size()) {
      ...
      for (GlyphRun& glyph_run : split_by_glyph_kind(span.text.substr(start, i - start))) {
        current.push_back(
            WordRun{std::move(glyph_run.text), bold, italic, span.strikethrough, false, glyph_run.kind});
      }
    }
  }
  flush();
...
void append_word(std::vector<InlineRun>& runs, float& width, const Word& word, const FontSet& fonts,
...
std::vector<TextLine> wrap_words_to_lines(const std::vector<Word>& words, const FontSet& fonts, float font_size,
                                           float max_width) {
...
TextLayoutResult layout_at_font_size(const std::vector<ContentBlock>& content, const FontSet& fonts,
                                      const AppConfig& config, float base_font_size, float max_width,
  ...
  for (size_t i = 0; i < content.size(); ++i) {
    ...
    if (is_thematic_break) {
      ...
      layout_block.list_indent =
          font_size * kListIndentPerLevelFraction * static_cast<float>(content_block.list_level);

      float marker_spacing = text_spacing(layout_block.list_marker_font_size);
      float marker_width = MeasureTextEx(fonts.regular, layout_block.list_marker.c_str(),
                                          layout_block.list_marker_font_size, marker_spacing)
                                .x;
      ...
      if (texture != nullptr && texture->width > 0 && texture->height > 0) {
        ...
        float scale_to_fit = std::min({max_width * kImageMaxWidthFraction / natural_width,
                                        available_height * kImageMaxHeightFraction / natural_height, 1.0f});
      ...
      auto measure_row = [&](const TableRow& row, bool force_bold) {
        ...
        for (size_t col = 0; col < column_count; ++col) {
          ...
          layout_block.table_column_width[col] =
              std::max(layout_block.table_column_width[col], line.width + 2.0f * cell_padding_x);
      ...
      layout_block.line_height = font_size * config.line_height_multiplier;
      float quote_indent =
          is_block_quote ? font_size * (kBlockQuoteBarWidthFraction + kBlockQuoteGapFraction) : 0.0f;
    ...
    result.block_height +=
        static_cast<float>(layout_block.lines.size()) * layout_block.line_height + block_extra_height;
...
TextLayoutResult compute_fitted_layout(const std::vector<ContentBlock>& content, const FontSet& fonts,
  ...
  TextLayoutResult result =
      layout_at_font_size(content, fonts, config, font_size, available_width, available_height, image_cache);

  while (font_size > config.min_font_size &&
         (result.block_height > available_height || result.block_width > available_width)) {
